...
def Book_Show(event):
    ...
    def modify_book(): # 수정 버튼을 눌렀을 때 원래 도서 정보의 내용이 바뀌어서 저장되게 하기 위한 메소드
        ...
        modify_list=np.append(textDesc) # 도서설명
...
def User_Add():
    ...
    def add_user(): # 등록 버튼을 누를시에 이름, 생년월일, 전화번호, 성별, 이메일, 사진의 정보를 받아 원래 회원 리스트에 추가해주는 메소드
        ...
        add_user_list=np.append() #가입일자 -> 따로 선언되어있는 변수 x
        add_user_list=np.append() #탈퇴일자 -> 따로 선언되어있는 변수 x
        add_user_list=np.append(textPicture)
...
def Rent_User_Search():
    ...
    def update_rent_situation(ind,isbn,phone,dat): # 선택 버튼을 눌렀을 시에 해당 회원의 대출 여부가 도서 대출 중으로 바뀌어 저장하는 메소드
        if US.get_IsRented(ind)==3: # 대출 진행 불가능 -> 해당 회원이 3권을 빌린 상태인지를 먼저 체크해야 함#US.get_IsRented(ind)로 불러오시면 
...
def Rent_State_Show(event):
    ...
    def get_rent_user_info(user): # 대출을 실행한 회원의 정보를 불러오는 메소드 -> 해당 책을 빌린 사람의 정보
